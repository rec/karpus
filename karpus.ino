...
ISR(TIMER1_COMPA_vect) {
    ...
    if (trig) { //if the karplus gets triggered
        ...
        for (int i = 0; i < SIZE; i++)
            delaymem[i] = random(); //fill the delay line with random numbers

        trig = false; //turn off the trigger

    } else {
        //the input to the delay line the output of the previous call of this function
        delaymem[locat++] = out;

        //this makes the delay line wrap around.
        // bound is the effective length of the line.
        if (locat >= bound)
            locat = 0;
        ...
        //1 pole IR filter, a smoothing function,
...
void loop() {
    ...
    for (int i = 0; i > -1; i = i + j) {
        //wrap entire karplus structure in this loop.
